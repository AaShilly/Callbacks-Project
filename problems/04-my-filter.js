...
let myFilter = function(arr, cb) {
    ...
    for (let i = 0; i < arr.length; i++){   //cycles through every element of the array
        let el = arr[i];    //sets each element of the array to the el variable

        if(cb(el)){
            filteredArr.push(el);   //adds the element if it satisfies the callback function condition
        }
    }
