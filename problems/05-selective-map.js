...
let selectiveMap = function(arr, cb1, cb2) {
    ...
    for (let i = 0; i < arr.length; i++){   //loops through all elements of array
        let el = arr[i];    //sets each element to the el variable

        if(cb1(el)){    //if the first callback returns true the newArray will add the element processed through the second callback function
            newArr.push(cb2(el));
        } else {
            newArr.push(el);    //if the first callback runs false the newArray will add the original element
        }
    }
