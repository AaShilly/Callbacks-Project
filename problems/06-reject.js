...
let reject = function(arr, cb) {
    ...
    for (let i = 0; i < arr.length; i++){   //loop through all array elements
        let el = arr[i];    //set each element to el variable

        if(!cb(el)){    //if el ran through callback function is false it will add it to the reject array
            rejectArr.push(el);
        }
    }
