...
let mySome = function(arr, cb) {
    for (let i = 0; i < arr.length; i++){   //loop through every element of passed in array
        let el = arr[i];

        let result = cb(el, i); //stores the callback back function return value to the result variable

        if(result){
            return true;    //if the result of element variable passing through cb function is true return true
        }
    }
