...
let count = function(arr, cb) {
    let trueCount = 0;  //true count variable

    //loop through each array element
    for (let i = 0; i < arr.length; i++){
        let el = arr[i]; //set each element of arr to el variable

        if(cb(el)){ //run each element through callback function
            trueCount++; //if callback function returns true it will increment counter;
        }
    }
