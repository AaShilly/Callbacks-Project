...
let exactly = function(arr, num, cb) {
    let trueCount = 0;  //sets counter for total true return values of elements passed through cb function
    //loops through every element
    for (let i = 0; i < arr.length; i++){
        let el = arr[i];
        //increments counter if current element returns true from cb function
        if(cb(el)){
            trueCount++;
        }
    }
    //checks whether total true return values of array elements equate num passed through function
    if (trueCount === num){
        return true;
    }
