...
let sentenceMapper = function(sentence, cb) {
    let words = sentence.split(" ");   //separates the sentence into an array of words

    let newWords = words.map((el) => cb(el)); //applies the cb function to each element and creates a new array with the results

    let newSentence = newWords.join(" "); //combines the new set of words into a new sentence
