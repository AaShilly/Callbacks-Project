...
let none = function(arr, cb) {
   ...
   for (let el of arr){
    //if any element returns true after being passed into the cb it will return false
        if(cb(el)){
            return false;
        }
   }
   //if the loop returns false for all arguments of the callback it will return true
    return true;
};
