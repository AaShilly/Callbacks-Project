...
let atMost = function(arr, max, cb) {
    //creates a filtered array that contain all values that return true from
    //being called intot he callback function
    let filtered = arr.filter ((el) => cb(el));
    //(if the amount if trues aka length of filtered arr are less than or equal to the max num it will return true
