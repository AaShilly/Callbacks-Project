...
let alternatingMap = function(arr, cb1, cb2) {
    //creates new array and uses the map method to alter the original argument array
    let newArr = arr.map((el, i) => {
        //if the index is 0 or even it will modify that element with cb1
        if(i % 2 === 0){
            return cb1(el);
        } else{ //if the index is odd it will modify that element with cb2
            return cb2(el);
        }
    });
